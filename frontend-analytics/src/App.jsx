    ...
    <div style={{ minHeight: '100vh', padding: '40px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel">
          ...
          <div className="stat-value">{(data[data.length-1]?.udpPackets / 1000).toFixed(1)}k <span style={{fontSize: '1rem', color: '#888'}}>pps</span></div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel">
          ...
          <div className="stat-value">{data[data.length-1]?.mlLatency.toFixed(1)} <span style={{fontSize: '1rem', color: '#888'}}>ms</span></div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel">
          <div className="stat-label"><Server size={16} color="#ff3366" /> Kafka Broker Health</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '30px' }}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 }} className="glass-panel" style={{ height: '400px' }}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              <YAxis stroke="#666" tick={{fill: '#666'}} domain={['dataMin - 10000', 'dataMax + 10000']} />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6 }} className="glass-panel" style={{ height: '400px' }}>
          <h3 style={{ marginBottom: '20px', color: '#fff', fontWeight: 500 }}>Redis Connection Pool</h3>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.slice(-10)}>
              ...
              <Bar dataKey="mlLatency" fill="#b026ff" radius={[4, 4, 0, 0]} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
